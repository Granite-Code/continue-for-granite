...
function getBaseConfig(
  ...
  try {
    ...
    const test_model = config.models.find((m: ModelDescription) => m.title == "TEST LLM");
    if (test_model)
      config.models = [test_model, ...config.models.filter((m: ModelDescription) => m.title != "TEST LLM")];
  } catch (e) {
